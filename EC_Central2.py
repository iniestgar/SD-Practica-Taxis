        ...
        try:
            ...
            if mensaje.startswith("ALTA"):
                ...
                if result:
                    ...
                    self.cursor.execute("UPDATE Taxis SET token = %s WHERE id_taxi = %s", (token, id_taxi))
                    self.log(f"Taxi existente actualizado: ID {id_taxi}, nuevo token generado.")
                else:
                    # Si no existe, insertar un nuevo registro
                    self.cursor.execute(
                        "INSERT INTO Taxis (id_taxi, token) VALUES (%s, %s)",
                        (id_taxi, token)
                    )
                    self.log(f"Taxi nuevo registrado: ID {id_taxi}, token generado.")
                ...
                self.log(f"Token y clave enviados al taxi {id_taxi}.")

            elif mensaje.startswith("AUTENTICAR"):
                # Autenticar taxi existente
                partes = mensaje.split(" ")
                if len(partes) < 2:
                    connstream.send("Error: Formato de mensaje inválido".encode())
                    connstream.close()
                ...
                if result:
                    # Generar un nuevo token y clave de cifrado
                    token = self.generar_token()
                    clave_cifrado = Fernet.generate_key().decode()

                    # Actualizar el token y clave en la base de datos
                    self.cursor.execute(
                        "UPDATE Taxis SET token = %s WHERE id_taxi = %s",
                        (token, id_taxi)
                    )
                    self.conn.commit()

                    # Actualizar en el diccionario tokens_taxis
                    self.tokens_taxis[token] = {
                        'id_taxi': id_taxi,
                        'clave_cifrado': clave_cifrado
                    }
                    ...
                    respuesta = json.dumps({"token": token, "clave": clave_cifrado})
                    connstream.send(respuesta.encode())
                    self.log(f"Taxi autenticado: ID {id_taxi}, token actualizado.")
                else:
                    connstream.send("Error: Taxi no encontrado.".encode())
                    self.log(f"Error: Intento de autenticación para taxi no existente: ID {id_taxi}.")

            else:
                connstream.send("Error: Comando no reconocido.".encode())
                self.log(f"Comando no reconocido recibido: {mensaje}")
